...
**根级（门户全局，`portal-neo-theme.css`，注入到 `:root`）**：
...
- `--neo-form-accent` / `--neo-form-accent-2` / `--neo-form-input-h` / `--neo-form-font-size` / `--neo-form-label-size` / `--neo-form-base` / `--neo-field-accent` — 见 [cmx-form-neo-skin.js](../../../../packages/cmx-data-comp/src/lib/cmx-form-neo-skin.js)
- `--neo-grid-accent` / `--neo-grid-accent-2/3/4` / `--revo-grid-bg` — 见 [cmx-grid-neo-skin.js](../../../../packages/cmx-data-comp/src/lib/cmx-grid-neo-skin.js)
...
- form 范例：[cmx-form-neo-skin.js](../../../../packages/cmx-data-comp/src/lib/cmx-form-neo-skin.js)
- grid 范例：[cmx-grid-neo-skin.js](../../../../packages/cmx-data-comp/src/lib/cmx-grid-neo-skin.js)
...
完整证据：[cmx-ui5-form.js](../../../../packages/cmx-data-comp/src/components/cmx-ui5-form.js)、[cmx-ui5-form.js](../../../../packages/cmx-data-comp/src/components/cmx-ui5-form.js)、[cmx-revo-grid.js](../../../../packages/cmx-data-comp/src/components/cmx-revo-grid.js)、[cmx-revo-grid.js](../../../../packages/cmx-data-comp/src/components/cmx-revo-grid.js)

### 3.4 激活 class 命名约定

- 主类：`cmx-<name>-neo`（如 `cmx-form-neo` / `cmx-grid-neo`）。所有 neo 专属样式都挂在 `:host(.cmx-<name>-neo) ...` 下面。
- 色调变体：`cmx-<name>-neo--<tone>`（如 `cmx-grid-neo--mint`、`cmx-grid-neo--violet`、`cmx-grid-neo--azure`）。仅 grid 有多个 tone；form 只有 `mint` 一个（[cmx-ui5-form.js](../../../../packages/cmx-data-comp/src/components/cmx-ui5-form.js)）。
...
证据：[cmx-skin-runtime.js](../../../../packages/cmx-data-comp/src/lib/cmx-skin-runtime.js)
...
- 证据：全库搜索 `registerNeoSkin` / `applyNeoSkin` / `registerSkin` / `applySkin` 命中文件仅 [cmx-skin-runtime.js](../../../../packages/cmx-data-comp/src/lib/cmx-skin-runtime.js)（**导出** `applyNeoSkin`，但**不是注册**——是执行）、[cmx-ui5-form.js](../../../../packages/cmx-data-comp/src/components/cmx-ui5-form.js)、[cmx-revo-grid.js](../../../../packages/cmx-data-comp/src/components/cmx-revo-grid.js) 三处 import。
...
2. **不要硬编码色值** — 一律 `var(--sap*)` / `var(--neo-*)` 派生（[page-style-guide.md](page-style-guide.md)）
3. **不要重复发明 `applyNeoSkin`** — 直接 import [cmx-skin-runtime.js](../../../../packages/cmx-data-comp/src/lib/cmx-skin-runtime.js)
...
7. **不要每页重定义 `--neo-*`** — 业务页要么继承 Portal 根，要么完整复制派生块（[page-style-guide.md](page-style-guide.md)）
...
10. **不要让 tone 覆盖全局 `--neo-cyan`** — tone 只应覆盖本组件的局部 `--neo-<name>-accent`，不污染根 token（参考 form 的 4 色循环是覆盖 `--neo-field-accent` 而非 `--neo-cyan`，[form line 147-150](../../../../packages/cmx-data-comp/src/lib/cmx-form-neo-skin.js)）
...
| 组件 | 皮肤文件 | 状态 |
|---|---|---|
| `cmx-ui5-form` | [cmx-form-neo-skin.js](../../../../packages/cmx-data-comp/src/lib/cmx-form-neo-skin.js) | 已发布 |
| `cmx-revo-grid` | [cmx-grid-neo-skin.js](../../../../packages/cmx-data-comp/src/lib/cmx-grid-neo-skin.js) | 已发布 |
| `cmx-status-tag` | [cmx-status-tag-neo-skin.js](../../../../packages/cmx-data-comp/src/lib/cmx-status-tag-neo-skin.js) | TDD 推进中(测试 [cmx-status-tag.test.js](../../../../packages/cmx-data-comp/src/components/__tests__/cmx-status-tag.test.js) 已存在) |
| `cmx-toolbar` | [cmx-toolbar-neo-skin.js](../../../../packages/cmx-data-comp/src/lib/cmx-toolbar-neo-skin.js) | TDD 推进中 |
...
| "全局 `__cmxDefaultXxxSkin` 必须设才能用" | **半对**。`applyNeoSkin()` 的 fallback 默认就是 'neo'([cmx-skin-runtime.js](../../../../packages/cmx-data-comp/src/lib/cmx-skin-runtime.js))。全局变量只是**覆盖**默认,缺省时仍然是 neo。 |
...
- 参照 `cmx-status-tag` 的双层 tone 模式(见 [cmx-status-tag-neo-skin.js](../../../../packages/cmx-data-comp/src/lib/cmx-status-tag-neo-skin.js))
- 适合:状态标签、徽章、告警条
