...
| 目录                       | 是什么                                                                                  | 技术栈                                |
| ------------------------ | ------------------------------------------------------------------------------------ | ----------------------------------- |
| `cmx-container`          | 后端**公用库 + 插件平台**（无可执行 server bin；各微服务经 `path = "../cmx-container/crates/..."` 跨 ws 引用） | Rust / axum / PostgreSQL / WASM    |
...
| `cmx-ontology`           | **本体平台微服务（`cmx-onto-server`，:8097）**，Palantir 式企业本体（对象/关系/接口/动作/函数），一芯多壳（onto-model / onto-store-pg / onto-app / onto-server）；内嵌壳 `cmx-onto-api` 规划留 container | Rust / axum                         |
...
| `cmx-ontology-graph`     | 本体图可视化编辑 Web Component（ER 图：富卡片对象 + 基数关系边 + 接口虚线），`cmx-ontology` 前端四区工作台引用 | TypeScript / 零框架零运行时依赖（独立 Git 仓） |
| `cmx-mega-sheet`         | 自研电子表格引擎（`<cmx-megasheet>` 自定义元素，零运行时依赖，含公式引擎 / 画布渲染 / XLSX·PDF·CSV IO） | TypeScript / Web Components / Vitest（**独立 Git 仓，不在 npm workspace**） |
| `cmx-portal-manager`       | 门户管理（UI5 WebComponents）                                                         | 原生 JS / **Vite 8** / UI5 / Lit   |
| `cmx-html-designer`        | HTML 页面设计器                                                                          | 原生 JS / **Vite 8**                |
| `packages/cmx-data-comp` | 共享数据组件库（表格/表单/数据集）                                                                 | 原生 JS Web Components / npm workspace / Vitest 4 |
| `packages/cmx-ui5-runtime` | 共享 UI5 + Tabler 运行时（前端挂 `/shared/`，Portal / Designer 均依赖，构建须**先建它**）        | 原生 JS / Vite 8 / npm workspace   |
| `packages/cmx-icon-resource` | 共享图标资源（Tabler / SAP 图标）                                                        | 静态资源 / npm workspace            |
***
...
| 目录                                                                    | 权威规范 / 技能目录                                                              |
| --------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| `cmx-container`                                                       | `cmx-container/AGENTS.md`（19 章）+ `cmx-container/.agents/skills/`（14 个）     |
| `cmx-portalservice` / `cmx-flowengine` / `cmx-report` / `cmx-rulesengine` / `cmx-mdm` / `cmx-model` / `cmx-ontology` / `cmx-data-auth` | 业务层薄，遵循 `cmx-container/AGENTS.md`（各仓 README 有架构说明）                       |
| `cmx-mega-sheet` / `cmx-ontology-graph`                                                      | 无（遵循就近代码风格；`README.md` 是唯一手册）                                          |
| `cmx-portal-manager` / `cmx-html-designer`                                | 技能 `cmx-components-guide`（`references/frontend-conventions.md`）         |
| `packages/cmx-data-comp`                                              | 技能 `cmx-components-guide`（封装源头，改导出 API 需评审）                            |
| `packages/cmx-ui5-runtime` / `packages/cmx-icon-resource`             | 无专项规范（改动影响 Portal + Designer 两端，须双端构建验证）                            |
| `cmx-portal` / `cmx-devops`                                           | ⚠️ **暂忽略**                                                                |

***

## 三、技能索引

需要时点开 `.agents/skills/<name>/SKILL.md` 查看，不要求预读。根目录 9 个（跨子项目）+ `cmx-container/.agents/skills/` 14 个（Rust 后端）。

### cmx-container（Rust 后端）
...
> `cmx-portal` / `cmx-devops` 仓未拉取，对应 `dev-api` / `ui-dev-list` 技能暂不触发。
...
2. **使用中文回复。**
3. **改** `package.json` / `Cargo.toml` **后先装再构建** — 先 `npm install` / `cargo check` 同步依赖，再 `build`，避免依赖未装导致构建失败。
4. **Rust 编译检查用** `cargo check` / `cargo clippy` — 禁止用 `cargo build`（耗时是 check 的数倍），仅产可执行产物（运行服务 / 跑集成测试 / release）时例外。
...
6. **新接口禁用可变路径段，禁用 PUT/PATCH/DELETE** —
   ...
   - ✅ 只对**新增接口**生效；既有接口保持原样。
7. **前端资源 / 数据资产必须从工作区真源发布** — `cmx-container/assets/<svc>/` 是**唯一真源**；主应用仓 `web/` / `data/` 下**与真源同名的顶层子目录**（如 `ui-html/`、`ui-native/`）是 `publish-assets.sh` 的发布产物，**禁止直接修改**（下次发布整目录替换即被覆盖）。

   - 改完 `cmx-container/assets/<svc>/` 后执行 `./scripts/publish-assets.sh <portal|model|mdm|flow|report|rules>` 同步（svc → 目标仓：portal→`cmx-portalservice`、model→`cmx-model`、mdm→`cmx-mdm`、flow→`cmx-flowengine`、report→`cmx-report`、rules→`cmx-rulesengine`）。
   - 同步粒度 = 真源 `web/`（`data/`）下的**顶层子目录**逐个整目录替换 + 顶层散文件覆盖拷入；目标目录下真源没有的其它内容（README、`core/` 等）**不删不改**。真源撤销某顶层子目录后，目标仓残留同名目录需**人工清理**（脚本仅提示）。
   - 归属自检：`python scripts/check-asset-ownership.py`（校验是否误改了发布产物而非真源）。
   - ✅ 仅约束**资源文件**（页面 / JS / HTML / JSON / BPMN / 种子数据 / 菜单 / 元数据），不约束各仓 Rust 源码 / `Cargo.toml` / `.env`。
   - ✅ 各主应用仓 `[assets]` 已**直指工作区**（运行时直接读 `cmx-container/assets/<svc>/`），发布脚本的拷贝仅用于打包 / 归档一致性——可选跑。
...
本工作区是**嵌套多 Git 仓**：根目录 1 个（前端 npm workspace）+ 10 个子仓独立（`cmx-container` / `cmx-portalservice` / `cmx-flowengine` / `cmx-report` / `cmx-rulesengine` / `cmx-mdm` / `cmx-model` / `cmx-ontology` / `cmx-data-auth` / `cmx-mega-sheet`，另有前端 TS 独立仓 `cmx-ontology-graph`）；`cmx-portal-manager` / `cmx-html-designer` / `packages/*` 非 Git，随根仓提交。`cmx-portal` / `cmx-devops` 暂未拉取，忽略。

> 根 `.gitignore` 已把各 Rust 子仓目录（`/cmx-container/`、`/cmx-portalservice/`、`/cmx-flowengine/`、`/cmx-report/`、`/cmx-rulesengine/`、`/cmx-mdm/`、`/cmx-model/` 等）排除；`cmx-mega-sheet/`、`cmx-ontology*`、`cmx-data-auth/` 未逐项列入 ignore，但**根仓零跟踪**，勿把它们 add 进根仓。

### 仓库清单

| 路径                                                  | 性质    | 远端                                  |
| --------------------------------------------------- | ----- | ----------------------------------- |
| `/`（根目录）                                            | Git   | 前端 npm workspace，`cmx-enterprise-portal.git`（openserver.cn） |
| `cmx-container/`                                    | Git   | **后端公用库**（无 server bin），`cmx-container.git`（openserver.cn） |
| `cmx-portalservice/`                                | Git   | **主应用 bin**（:8080），`cmx-portalservice.git`（gitee） |
| `cmx-flowengine/` / `cmx-report/` / `cmx-rulesengine/` / `cmx-mdm/` / `cmx-model/` / `cmx-mega-sheet/` | Git   | `cmx-<svc>.git`（gitee，均已建）        |
| `cmx-ontology/` / `cmx-data-auth/` / `cmx-ontology-graph/` | Git   | `cmx-<svc>.git`（gitee，`warpdrivelabs_ai` org） |
| `cmx-portal/` / `cmx-devops/`                       | Git   | ⚠️ 暂未拉取                              |
| `cmx-portal-manager` / `cmx-html-designer` / `packages/cmx-data-comp/` / `packages/cmx-ui5-runtime/` / `packages/cmx-icon-resource/` | 非 Git | 随根仓                                  |

### 操作要点

1. **根目录慎用** `git add .` / `-A` — 会把子仓内部变更**递归纳入**根仓暂存区，跨仓污染。提交前 `git status -sb` 确认范围，改用 `git add <具体路径>`。
...
5. **改 cmx-container 公用库 API 必须下游验证** — 8 个下游仓（`cmx-portalservice` / `cmx-flowengine` / `cmx-report` / `cmx-rulesengine` / `cmx-model` / `cmx-mdm` / `cmx-ontology` / `cmx-data-auth`）都经 `path = "../cmx-container/crates/..."` 反向引用公用库；改后**至少**在主应用 + 一个引擎各跑 `cargo check`，动到被多仓共用的 crate（core / sql / web / rpc 等）则**逐仓全跑**：
   ```bash
   cd cmx-portalservice && cargo check
   cd cmx-flowengine && cargo check
   ```
   > `cmx-data-auth` 还跨仓引用 `cmx-rulesengine` 的 rule-feel / rule-model / rule-engine crate——改 rulesengine 这三个 crate 时须到 `cmx-data-auth` 补跑 `cargo check`。
...
### 7.1 前端联调（webapp-testing / Playwright）
...
主服务 `cmx-portal-server` 在 `cmx-portalservice` 仓库，读它自己的 `.env` / toml。

- **只读** `cmx-portalservice/.env`（蓝本是 `cmx-container/.env`，凡指向 cmx-container 资源的相对路径前缀 `../cmx-container/`，如 `WEB_FOLDER="../cmx-container/crates/web/web-folder"`），取 `CONFIG_FILE`（当前 `./portal-server-dev.toml`）确定生效的 toml。各引擎微服务同理：`<svc>-server-dev.toml`（dev）/ `<svc>-server.toml`。
...
```bash
cd cmx-portalservice && ./portal.sh          # 开发模式（debug，增量编译）
./portal.sh --release                         # 发布模式
```

七个引擎微服务（flow :8091 / report :8092 / model :8093 / rules :8094 / mdm :8095 / onto :8097 / dataauth :8098）各仓各自 `*.sh`（`flow.sh` / `report.sh` / `model.sh` / `rules.sh` / `mdm.sh` / `onto.sh` / `dataauth.sh`）启动；门户经 `[center_client.services]` 反代。前端资产真源在 `cmx-container/assets/<svc>/`，由各仓 toml `[assets]` 直指（见 §四、7）。
...
根目录是 npm workspace（`packages/*` + `cmx-html-designer` + `cmx-portal-manager`），统一用 `-w <包名>` 或根脚本执行。**改 `package.json` 后先 `npm install` 再 build。**
...
> `cmx-mega-sheet` **不在** 根 workspace：进目录单独跑（`npm test` / `npm run build` / `npm run typecheck`，vitest + tsc）。
>
...
| `.qoder/repowiki/knowledge/zh/`       | **模块知识库（自动生成）**——7 个条目 + `_index.yaml`：`CMX 企业门户前端 Monorepo`（下挂 Designer / 共享包 / Portal Manager / mega-sheet 等子模块）、配置体系（TOML + Nacos）、日志体系（tracing / chassis）、错误处理体系、样式体系（UI5 + Neo）、构建发布体系、npm 依赖管理 | **改某模块前**先读该条目的 `概述` / `架构设计` / `技术栈` / `编码规范` / `特殊配置与命令` |
| `documents/`                          | **人工方案库（唯一真源，见 §四、8）**——根 27 个方案散文件 + `plans/`、`MDM主数据管理平台/`（20）、`复杂问题分析/`（3）、`技术债/`（3）                                    | **新方案 / 计划写这里**，按 `plan-naming` 命名                     |
| `docs/`                              | **历史 / 专题设计资料区**——根 60 个散文件（32 md + 27 html + 1 txt：DCT/DOC 元数据、MDM、Rust-WASM、SAP FI/CO 参考、报表 / 权限 / 编码引擎等）+ `CMXPortalManager+CMXHTMLDesigner-模型体系文档/`（20）、`assets/`、`.cache/`（脚本抓取缓存，**勿手改**） | **查历史设计**用；❌ 不要往里写新方案                                 |
| 子仓 `docs/` + `README.md`             | 各仓自身架构 / API / 测试手册（如 `cmx-container/docs/{sql,assessments}`、`cmx-flowengine/docs/{usage,biz-test,agent-flows}`、`cmx-report/docs/summary`、`cmx-rulesengine/docs/full-test`） | 后端细节查这里；跨子项目材料**不要**塞进去（§六、4）                       |

**红线**：

- `.qoder/repowiki/` 全是**生成物**——不手工编辑（重新生成即覆盖）；与代码冲突时**以代码为准**，并按 §四、8 把纠偏结论写进 `documents/`。
