...
| `POST /action-outbox/{id}/dispatched` | 投递完回写状态（成功→dispatched，失败→failed） | 路径：`id`；body：`{ ok*: true/false, error? }` | —（dispatcher 回调） |
...
- `GET /action-logs`、`GET /action-outbox`、`GET /action-outbox/config`、`POST /action-outbox/dispatch`、`POST /action-outbox/{id}/dispatched` —— 审计与 Outbox 投递闭环（**定时自动投递已内置**，手动 dispatch 主要留运维补投；QA 脚本全覆盖）。
